...
public final class StreetSearcher {
    ...
    // Comparator to compare distances of vertexes
    static class VertexComparator implements Comparator<Vertex<String>> {

        public int compare(Vertex<String> v1, Vertex<String> v2) {
            Double d1 = graph.getDistance(v1);
            Double d2 = graph.getDistance(v2);
    ...
    private static void findShortestPath(String startName, String endName) {
        graph.initializeDists(MAX_DISTANCE);

        Vertex<String> start = vertices.get(startName);
        Vertex<String> end = vertices.get(endName);
        PriorityQueue<Vertex<String>> pq =
                new PriorityQueue<>(vertices.size(), new VertexComparator());

        graph.setDistance(start, 0);
        pq.add(start);

        while (!pq.isEmpty()) {
            Vertex<String> vert = pq.poll();
            double vertdist = graph.getDistance(vert);

            if (!graph.hasVisited(vert)) {
                for (Edge<String> edg : graph.outgoing(vert)) {
                    Vertex<String> stop = graph.to(edg);
                    double edgdist = (Double) graph.label(edg);
                    double stopdist = graph.getDistance(stop);
                    boolean shorter =  (vertdist + edgdist) < stopdist;

                    if (!graph.hasVisited(stop) && shorter) {

                        graph.setDistance(stop, vertdist + edgdist);
                        graph.label(stop, edg);
                        pq.add(stop);
                    }
                }
            }
