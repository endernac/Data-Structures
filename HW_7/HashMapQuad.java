...
// This implementation uses a quadratic probing approach to
// collision resolution. It uses the has overall structure as
// Linear probing hashmap (LinHashMapQuad in maps), by using an array
...
//
// The quadratic hash resolution method is very similar to the linear
// hash resolution method. Every time that there is a collision, I
// increase the index by the square of the next integer. One important
// thing to note about this approach is that it might result in cycles
// if the array is too small, so it is even more important to grow the
// array once 50% capacity has been reached. One advantage of quadratic
// probing is that it helps avoid clustering due to multiple collisions.
//
 ...
 * @param <V>
 */
public class HashMapQuad<K, V> implements Map<K, V> {
    ...
    private HashMapQuad(int tablesize) {
        m = tablesize;
        table = new Entry[m];
        size = 0;
    }
    ...
    private void resize(int num) {
        ...
        HashMapQuad<K, V> temp = new HashMapQuad<>(nextPrime(num));
        for (int i = 0; i < m; i++) {
    ...
    private Entry find(K k) {
        ...
        int q = 1;
        for (int i = hash(k); table[i] != null; i = (i + (q++) * (q++)) % m) {
            Entry<K, V> e = table[i];
